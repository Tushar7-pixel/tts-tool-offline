    ...
    const handleOpenDetail = async (book: BookWork) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (fetchedDesc) {
                    setActiveDetail((prev) => (prev ? { ...prev, description: fetchedDesc } : null));
                }
            }
        } catch (err) {
                ...
                <div className="p-3 border-b border-inherit bg-black/5 flex items-center justify-between gap-3 shrink-0">
                    <div className="flex rounded-md p-0.5 bg-black/10 border border-inherit">
                        <button
                            ...
                            className={`text-xs px-3 py-1 rounded font-semibold transition cursor-pointer ${activeTab === "genres"
                        ...
                        <button
                            ...
                            className={`text-xs px-3 py-1 rounded font-semibold transition cursor-pointer ${activeTab === "authors"
                                    ? "bg-amber-400 text-black font-bold shadow-xs"
                                    : "app-muted hover:opacity-100"
                                }`}
                        >
                        ...
                        {activeTab === "genres"
                            ? GENRES.map((g) => (
                                <button
                                    ...
                                    className={`text-[11px] px-2.5 py-1 rounded-full whitespace-nowrap transition cursor-pointer flex items-center gap-1 border ${selectedGenre === g.id
                            ...
                            : PRESET_AUTHORS.map((a) => (
                                <button
                                    ...
                                    className={`text-[11px] px-2.5 py-1 rounded-full whitespace-nowrap transition cursor-pointer border ${activeAuthor.toLowerCase() === a.name.toLowerCase()
                                            ? "bg-amber-400 text-black font-bold border-amber-400 shadow-xs"
                                            : "app-btn border-inherit"
                                        }`}
                                >
                        ...
                        activeDetail ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 items-start pt-1">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-2.5">
                                        <div>
                                            <h3 className="text-base sm:text-lg font-bold">{activeDetail.title}</h3>
                                            <p className="text-xs app-muted">
                                                By {activeDetail.authors?.join(', ') || 'Unknown Author'}
                                                {activeDetail.year && <span> • First published {activeDetail.year}</span>}
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-2 pt-1">
                                            <button
                                                type="button"
                                                onClick={() => openOcean(activeDetail.title, activeDetail.authors?.[0])}
                                                ...
                                                {detailLoading ? (
                                                    <div className="flex items-center gap-2 py-3 text-amber-500">
                                                        <div
                                                            ...
                                                            role="status"
                                                        />
                                                        <span className="text-xs">Loading summary from Open Library...</span>
                                                    </div>
                                                ) : activeDetail.description ? (
                                                    activeDetail.description
                                                ) : (
                                                    <span className="app-muted italic">
                                                        No synopsis is available for this title in Open Library's public records.
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                            ...
                            books.length === 0 ? (
                                <div className="py-16 text-center app-muted text-xs space-y-1">
                                    ...
                                    <p className="text-[11px] opacity-75">Try checking another genre or author name.</p>
                                </div>
                            ) : (
                                        ...
                                        >
                                            <div className="w-12 h-16 bg-black/10 rounded border border-inherit flex items-center justify-center shrink-0 overflow-hidden">
                                                {b.cover_id ? (
                                                    ...
                                                    <span className="text-[10px] app-muted text-center px-0.5">No cover</span>
                                                )}
                                            </div>

                                            <div className="flex-1 min-w-0">
                                                <h4 className="text-xs font-bold truncate" title={b.title}>{b.title}</h4>
